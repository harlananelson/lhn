...
# ---- Persist ----
# write_all() as a safety net. The extract methods above
# (dict2pyspark, create_extract, entityExtract, write_index_table) all
# call _auto_write internally, so every one of those items is already
# in Hive and clean. write_all with the default skip_clean=True only
# writes items whose .df was mutated after the last persist. In the
# pipeline above, only persontenant is dirty -- mycodes / mycodesVerified
# / myEncounter / myCohort report "SKIPPED (already persisted)".
# Pass skip_clean=False if you want every table re-written regardless.
...
6. **Manual `e.foo.df = ...` assignment** → does NOT auto-write. The `df` setter marks the item dirty, so either `e.foo.write()` or a closing `e.write_all(...)` picks it up; without one of those the change lives only in this Python session.
