...
| `schema_type` | Config Key | Key in `000-config.yaml` → `schemas` dict that maps to a Spark catalog schema name | `RWDSchema` → `real_world_data_ed_cohort` |
...
```yaml
schemas:
  RWDSchema: real_world_data_ed_cohort    # Logical → Physical
...
Lookup: config_dict['schemas']['RWDSchema']
                    ↓
Returns: 'real_world_data_ed_cohort' (Spark catalog schema)
                    ↓
Validated: database_exists('real_world_data_ed_cohort')
```
