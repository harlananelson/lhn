...
- `real_world_data_ed_cohort` - ED cohort data
...
schemas:
  RWDSchema: real_world_data_ed_cohort    # Source data
...
**Key insight**: The `schemas` dictionary maps *logical names* (like `RWDSchema`) to *physical Spark schemas* (like `real_world_data_ed_cohort`). Your code uses the logical names; the config determines where they point.
...
3. Load each referenced config (config-RWD.yaml, etc.)
   - config_dict now contains:
     - schemas: {RWDSchema: 'real_world_data_ed_cohort', ...}
...
For each entry in schemas:
  ...
  Processing RWDSchema → real_world_data_ed_cohort:
    1. Find matching callFunc where schema_type == 'RWDSchema'
       → Found: RWDcallFunc
    2. Check: database_exists('real_world_data_ed_cohort')?
       → Yes, proceed
...
print(condition_item.location)   # 'real_world_data_ed_cohort.condition'
...
# Ensure 'real_world_data_ed_cohort' is listed
```
